    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _, masks) in pbar:  # batch ------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if device.type == 'mlu':
                    pred=tuple([item.cpu() for item in sublist] for sublist in pred)
            
                    loss, loss_items = compute_loss(pred, targets,masks=masks.float())
                else:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default='/work/yolov5-master/data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
...
if __name__ == "__main__":
    opt = parse_opt()
    opt.weights ="/work/yolov5-master/yolov5s-seg.pt"
    opt.project ="out4/train"
    import yaml 
    with open("/work/yolov5-master/data/coco128-seg.yaml", "r") as file:
        data = yaml.safe_load(file)
    data["path"] = os.environ["DATASETS_PATH"]+"/coco128-seg"

    with open("/work/yolov5-master/data/coco128-seg.yaml", "w") as file:
        yaml.dump(data, file)
    opt.data = "/work/yolov5-master/data/coco128-seg.yaml"
    opt.batch_size=int(os.environ["BATCH_SIZE"])
    opt.epochs = int(os.environ["MAX_EPOCHS"])
    main(opt)
