...
if __name__ == "__main__":
    ...
    if "CLASS_LABELS" in os.environ:
        ...
        num_classes = int(os.environ["CLASS_NUM"])
        labels = ["label_%d"%i for i in range(num_classes)]
    else:
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "/work/deeplabv3-plus-pytorch-main/model_data/deeplab_mobilenetv2.pth"
    #---------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    save_dir = str(increment_path(Path('out2/train') / "exp", exist_ok=False))
    if not os.path.isdir(save_dir):
         os.makedirs(save_dir)
    with open(save_dir+"/class_name.txt", 'w') as file:
        file.write('\n'.join(labels))
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    VOCdevkit_path  = os.environ["DATASETS_PATH"]+"/VOCdevkit"
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = Unfreeze_batch_size, num_workers = num_workers, pin_memory=True,
                                drop_last = True, collate_fn = deeplab_dataset_collate, sampler=train_sampler)
    gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = Unfreeze_batch_size, num_workers = num_workers, pin_memory=True, 
    ...
    if model_path != '':
        ...
        model = qt.adaptive_quantize(model=model, steps_per_epoch=len(gen), bitwidth=16)
        model.to(device)
        #------------------------------------------------------#
        #   显示没有匹配上的Key
        #------------------------------------------------------#
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Mlu, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank)
