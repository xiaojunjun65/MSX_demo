...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if "CLASS_LABELS" in os.environ:
        ...
        class_num = len(labels)
    with open(opt.save_dir+"/weights/"+"class_name.txt", 'w') as file:
        file.write('\n'.join(labels))
    ...
    data_dict['train'] = os.path.join(TASK_ID, 'train.txt')
    data_dict['val'] = os.path.join(TASK_ID, 'val.txt')
    data_dict['test'] = None
...
def parse_opt(known=False):
    ...
    # SDK
  #  parser.add_argument('--save_dir', type=str, required=True, help='train dataset path')
    ...
    parser.add_argument("--label_name", nargs="+",  help='category order')
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_wandb_resume(opt) and not opt.evolve:  # resume an interrupted run
        ...
        log_dir = os.path.dirname(opt.save_dir)
